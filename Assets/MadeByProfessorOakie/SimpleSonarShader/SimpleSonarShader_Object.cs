...
public class SimpleSonarShader_Object : MonoBehaviour
{
    ...
    private void Start()
    {
       ...
       GameManager.Instance.sonarObject.Add(this);
    }
    ...
    private IEnumerator Light()
    {
        ...
        for (; ; )
        {
            ...
            {
                lastTime = Time.time;
                for (int i = 0; i < GameManager.Instance.collectibles.Count; i++)
                {
                    if(Vector3.Distance( GameManager.Instance.collectibles[i].transform.position,transform.position) < 150)
                        StartSonarRing(GameManager.Instance.collectibles[i].transform.position, 0.5f);
                }
            }
            yield return null;
        }
